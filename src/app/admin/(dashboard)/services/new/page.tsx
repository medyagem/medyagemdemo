    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-20">
       <div className="flex justify-between items-center border-b border-border pb-6">
         <div className="flex items-center gap-4">
           ...
           <div>
             <h2 className="text-2xl font-bold text-heading">Yeni Hizmet Oluştur</h2>
             <p className="text-desc mt-1">Gelişmiş içerik ve SEO ayarları ile yeni hizmet ekleyin.</p>
           </div>
         </div>
         <Button onClick={handleSubmit} disabled={loading} className="gap-2">
           <Save className="w-4 h-4" /> {loading ? "Kaydediliyor..." : "Kaydet ve Yayınla"}
         </Button>
       </div>

       <SEOImporter type="service" onImport={handleSEOImport} />
            ...
            <div className="bg-surface border border-border rounded-xl p-8 space-y-6 shadow-sm">
               ...
               <div className="space-y-4">
                 <div className="space-y-2">
                   ...
                   <input name="name" onChange={handleChange} value={formData.name} required className="w-full bg-background border border-border p-3 text-white focus:border-brand-primary rounded-lg transition-all" placeholder="Örn: SEO Danışmanlığı" />
                 </div>
                 <div className="space-y-2">
                   <label className="text-sm font-medium text-main">URL (Slug) - Otomatik Oluşturulur</label>
                   <input name="slug" onChange={handleChange} value={formData.slug} required className="w-full bg-background border border-border p-3 text-white focus:border-brand-primary rounded-lg font-mono text-sm opacity-80" placeholder="orn: seo-danismanligi" />
                 </div>
               </div>
            </div>
